...
public class MemoryRepository implements IRepository, IRepositoryObject {
    ...
    @Override
    public Collection<MObject> findByAtt(final SmClass cls, final String att, final Object val) {
        Collection<MObject> results = new ArrayList<>();
        this.cache.findByAtt(cls, att, val, results);
        return results;
    }

    @objid ("bd839b20-92d7-11e1-81e9-001ec947ccaf")
    @Override
    public Collection<MObject> findByClass(final SmClass cls) {
        Collection<MObject> results = new ArrayList<>();
        this.cache.findByClass(cls, results);
        return results;
    }

    @objid ("bd85fd20-92d7-11e1-81e9-001ec947ccaf")
    @Override
    public SmObjectImpl findById(final SmClass cls, final UUID siteIdentifier) {
