...
public class DefaultAccessHandle implements IAccessHandle {
    ...
    @Override
    public boolean checkAccessFor(final SmObjectImpl obj, final SmAttribute att, final SmObjectImpl val) throws AccessDeniedException {
    ...
    @Override
    public boolean checkAccessFor(final SmObjectImpl obj, final SmDependency dep, final SmObjectImpl val) throws AccessDeniedException {
        boolean modifiable = checkAccess(obj, dep, val);
    ...
    private boolean checkAccess(final SmObjectImpl obj, final SmDependency dep, final SmObjectImpl val) throws AccessDeniedException {
        final boolean modifiable = isModifiable(obj);
        if (!modifiable) {
            if (!obj.getClassOf().hasDirectiveInGraph(SmDirective.NOREADONLY)) {
