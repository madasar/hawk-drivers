...
public class MemoryManager implements IMemoryManager, Runnable {
    ...
    private Collection<Map<UUID,ISmObjectData>> caches = new ArrayList<>();
    ...
    public static MemoryManager get() {
        if (instance==null) synchronized (MemoryManager.class){
            if (instance==null) 
    ...
    public synchronized void addManagedCache(Map<UUID, ISmObjectData> dataCache) {
    ...
    public synchronized void removeManagedCache(Map<UUID, ISmObjectData> dataCache) {
    ...
    synchronized void freeMemory(MemoryUsage memoryState) {
        ...
        for (Map<UUID, ISmObjectData> c : this.caches) {
    ...
    private int freeMemory(Map<UUID, ISmObjectData> cache, int toRemoveIdx) {
        int nb = 0;
        Iterator<Entry<UUID, ISmObjectData>> it = cache.entrySet().iterator();
        
        while (it.hasNext() ) {
            Entry<UUID, ISmObjectData> entry = it.next();
