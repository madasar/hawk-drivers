 ...
 * <p>
 * A SmObjectImpl holds a Java weak reference to its SmObjectData data structure. Therefore data can be garbaged at any moment.
 * Before being garbaged, data are in fact swapped to the disk for future re-use by a data cache holder defined in the upper level.
 * <p>
...
public abstract class SmObjectImpl implements ISmMeta, ISmStorable, MObject, Serializable, EObject {
    ...
    @objid ("1b5dc642-c239-457d-b104-5a81fdfc99a2")
    private UUID uuid;
     ...
     * Cette methode permet d'ajouter une dependance. Si un meta-objet est associe a? l'objet courant la methode passe la main au
     * meta-objet.<br>
     * Exemple: <br>
    ...
    /**
     * Get the value(s) of a dependency via the MetaObject. The returned value is a single possibly null object if the dependency is
     * 'simple' or an unmodifiable possibly empty list if the dependency is multiple.
     */
    ...
    public Object setDepVal(final SmDependency dep, final int index, final SmObjectImpl value) {
        ...
        // Prevent ref from being twice in the list
        this.eraseDepVal(dep, value);
        
        // Erase old value
        SmObjectImpl oldVal = this.eraseDepVal(dep, index);
        
        // do the real job
        getData().getMetaOf().setObjDepVal(this, dep, value, index);
     ...
     * Cette methode permet d'enlever une dependance. Si un meta-objet est associe a? l'objet courant la methode passe la main au
     * meta-objet.<br>
     * Exemple: <br>
     * 
     * <pre>
    ...
    private void propagateAppendToSymetric(final SmDependency dep, final SmObjectImpl value) {
        ...
        if (value != null && symetricDep != null) {
            ...
            if (getData().getRepositoryObject().isPersistent(symetricDep) && !symetricDep.isMultiple()) {
    ...
    public Object getAttVal(final SmAttribute att) {
        /*
         * if (isShell() && !(att.getName().equalsIgnoreCase("name") || att.equals(SmObjectData.Metadata.statusAtt()))) { // uuid,
         * name, status return null; }
         */
    ...
    public SmClass getClassOf() {
        if (this.dataRef == null || this.dataRef.get() == null) {
            return SmClass.getClass(SmLiveId.getClassId(this.liveId));
        }
        return getData().getClassOf();
    }
    ...
    public final boolean hasStatus(final long flag) {
        // optimization: should call MetaOf.getAttVal(..) to get pstatus
        long pStatus = getSmStatusFlags();
        StatusState ret = SmStatus.areAllSet(pStatus, flag);
        ...
        if (ret == StatusState.UNDEFINED) {
            while (ret == StatusState.UNDEFINED) {
                ...
                ret = SmStatus.areAllSet(pStatus, flag);
            }
        }
    ...
    public final void init(final UUID uuid, final long liveId) {
        ...
        //    this.hardData.init(uuid, liveId);
     ...
     * <p>
     * <b>Note :</b> the data is stored by {@link WeakReference}.
     * <b>Keep a reference</b> on the data to not let it garbage collected !
    ...
    public void setMetaOf(final IMetaOf metaObject) {
        getData().setMetaOf(metaObject);
    }

    @objid ("0082763e-9fc0-1f4f-9c13-001ec947cd2a")
    @Override
    public void setRepositoryObject(final IRepositoryObject createObject) {
    ...
    public void setPStatus(long trueFlags, long falseFlags, long undefFlags) {
        ...
        setAttVal(SmObjectData.Metadata.statusAtt(), newValue);
    }
     ...
     * <p>
     * Beware some flags may not be defined on this model object.
     * This is a low level method, it is recommanded to use {@link #getStatus()} in most case.
     * @see #getStatus()
