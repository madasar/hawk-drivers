...
public class BindingCreationExpert extends DefaultLinkExpert {
    ...
    public boolean canSource(final MClass link, final MClass fromMetaclass) {
        if (CollaborationUse.class.isAssignableFrom(Metamodel.getJavaInterface(fromMetaclass))) {
            return true;
        }
        
        if (Attribute.class.isAssignableFrom(Metamodel.getJavaInterface(fromMetaclass))
                || AssociationEnd.class.isAssignableFrom(Metamodel.getJavaInterface(fromMetaclass))
                || BindableInstance.class.isAssignableFrom(Metamodel.getJavaInterface(fromMetaclass))
                || ConnectorEnd.class.isAssignableFrom(Metamodel.getJavaInterface(fromMetaclass))
                || Parameter.class.isAssignableFrom(Metamodel.getJavaInterface(fromMetaclass))) {
            return true;
        }
        return false;
    }
    ...
    public boolean canSource(final MObject from, final MObject owner) {
        if (from instanceof CollaborationUse) {
            ...
            if (fromOwner == null) {
                ...
                return fromOwner.getOwnedCollaborationUse().contains(owner);
            } else {
                return false;
            }
            ...
            final Operation op = (Operation) from.getCompositionOwner();
            if (op == null) {
                return false;
            } else if (owner == null) {
                return (op.getOwnedCollaborationUse().size() > 0);
            } else if (owner instanceof CollaborationUse) {
                return op.getOwnedCollaborationUse().contains(owner);
            } else {
                return false;
            }
