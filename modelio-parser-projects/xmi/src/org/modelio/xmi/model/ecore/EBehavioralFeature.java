    ...
    private void setMethod(Operation objingElt) {
        ...
        for (org.eclipse.uml2.uml.Behavior behavior : ecoreElement.getMethods()){
            if (behavior.getOwner() instanceof  org.eclipse.uml2.uml.Transition){
                ...
                if (obBehavior != null) {
                    Dependency dependency = mmService.getModelFactory().createDependency();

                    try {
                        dependency.getExtension().add(mmService.getStereotype(XMIProperties.modelerModuleName, IModelerModuleStereotypes.UML2METHODREFERENCE, dependency.getMClass()));
