...
public class EPort extends EFeature {
        ...
        for (Object end : this.ecoreElement.getEnds()){
            if (end instanceof org.eclipse.uml2.uml.ConnectorEnd){
                ...
                if ( prop != null ){
                    ...
                    Port portClone = clonePort(objingElt);
    ...
    private Port clonePort(final Port objingElt) {
        ...
        Port result = mmodelservices.getModelFactory().createPort();
        result.setRepresentedFeature(objingElt);
        for(ProvidedInterface provided : objingElt.getProvided()){
            result.getProvided().add((ProvidedInterface) MTools.getModelTool().cloneElement(provided));
        }
        
        for(RequiredInterface required : objingElt.getRequired()){
            result.getRequired().add((RequiredInterface) MTools.getModelTool().cloneElement(required));
        }
    ...
    private void setRedefinedPort(final Port objingElt) {
        ...
        if ((redefinedPorts != null) && (redefinedPorts.size() > 0)){
            ...
            if (redefinedPort instanceof ModelElement){
                objingElt.setRepresentedFeature((ModelElement) redefinedPort);
            }
        }
    }
