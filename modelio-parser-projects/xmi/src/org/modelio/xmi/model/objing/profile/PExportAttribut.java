...
public class PExportAttribut implements IExportProfileElement {
    ...
    public void visit() {
        ...
        if (this.objingElt.getParamNumber().equals("0")){
            attr = stereotype.getOwnedAttribute(name,  GenerationProperties.getInstance().getEcoreUMLTypes().getBoolean());
            if (attr == null)
                attr = stereotype.createOwnedAttribute(name,  GenerationProperties.getInstance().getEcoreUMLTypes().getBoolean());
            attr.setLower(1);
            attr.setUpper(1);
        }else{
        
            attr = stereotype.getOwnedAttribute(name,  GenerationProperties.getInstance().getEcoreUMLTypes().getString());
            if (attr == null)
                attr = stereotype.createOwnedAttribute(name,  GenerationProperties.getInstance().getEcoreUMLTypes().getString());
