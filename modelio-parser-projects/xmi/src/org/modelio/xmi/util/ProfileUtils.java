...
public class ProfileUtils {
    ...
    private static MetaclassReference createReference(final org.eclipse.uml2.uml.Stereotype ecoreElt) {
        MetaclassReference objReference  = (MetaclassReference) ReverseProperties.getInstance().getMModelServices().getModelFactory().createElement("MetaclassReference");
    ...
    public static void addExtension(final ModelElement objModelElement, final org.eclipse.uml2.uml.Stereotype stereotype) {
        if (!ObjingEAnnotation.isReference(stereotype)){
            ...
            for (Stereotype existingStereotype : importedStereotypes){
                if (objModelElement.getMClass().hasBase(SmClass.getClass(existingStereotype.getBaseClassName()))){
            ...
            if ((notApply) && !(ObjingEAnnotation.isReference(stereotype))) {
                ...
                try {
                    objStereotype = ReverseProperties.getInstance().getMModelServices().getStereotype(
                            name, objModelElement.getMClass() );
    ...
    public static Profile createObjProfile(final org.eclipse.uml2.uml.Profile ecoreElement) {
        ...
        if (objProfile == null){
            ...
            if ( (ecoreElement.getName() != null) 
                    && (ecoreElement.getName().equals("LocalProfile"))){

                for (MObject modelRoot : ReverseProperties.getInstance().getModelRoots()){
                ...
                if  (ids.size() > 0){
                    ...
                    objProfile = (Profile)  ReverseProperties.getInstance().getMModelServices().findById(SmClass.getClass(Profile.class), UUID.fromString(ids.get(0)));
                    if (objProfile == null){
                        for (MObject module : ReverseProperties.getInstance().getLibraryRoots()){
                ...
                if (objProfile == null){
                    objProfile = ReverseProperties.getInstance().getMModelServices().getModelFactory().createProfile();
    ...
    private static Object createObjStereotype(final org.eclipse.uml2.uml.Stereotype ecoreElement) {
        ...
        List<String> ids = ObjingEAnnotation.getObjingIDs(ecoreElement);
        for (String id : ids){
            Stereotype result = (Stereotype)  ReverseProperties.getInstance().getMModelServices().findById(SmClass.getClass(Stereotype.class), UUID.fromString(id));
            if (result != null)
                results.add(result);        
        }

        if (results.size() == 0){
            baseClasses = ObjingEAnnotation.getBaseClass(ecoreElement);
            ...
            for (String baseClass : baseClasses){
                ...
                Stereotype result = (Stereotype) ReverseProperties.getInstance().getMModelServices().getModelFactory().createElement("Stereotype");
    ...
    private static void attach(final Stereotype objingSterotype, final org.eclipse.uml2.uml.Stereotype ecoreSterotype) {
        Object owner =  TotalImportMap.getInstance().get(ecoreSterotype.getOwner()); 
        ...
        if ((owner != null) 
            ...
            for (MObject module : ReverseProperties.getInstance().getModelRoots()){
                if ((module instanceof ModuleComponent) && (module.getName().equals("LocalModule"))){
                    for(Profile profile : ((ModuleComponent) module).getOwnedProfile()){
                        if (profile.getName().equals("LocalProfile")){                        
                            profile.getDefinedStereotype().add(objingSterotype);                          
                        }
                    }
                }
            }
    ...
    private static List<org.eclipse.uml2.uml.Stereotype> getParent(final org.eclipse.uml2.uml.Stereotype stereotype) {
        List<org.eclipse.uml2.uml.Stereotype> result = new ArrayList<org.eclipse.uml2.uml.Stereotype>();
    ...
    private static void setIconsProperties(final org.eclipse.uml2.uml.Stereotype stereotype, final Stereotype obStereotype) {
        ...
        GProject project = GProject.getProject(obStereotype);

        for (GModule module : project.getModules()){
            if (module.getName().equals(moduleName)){
                IModuleHandle moduleHandle = module.getModuleHandle();
                if (moduleHandle != null){

                    Path  moduleResPath = moduleHandle.getResourcePath().toAbsolutePath();
    ...
    private static NoteType createNoteType(final Stereotype stereotype, final Property ecoreElement) {
        NoteType noteType  = (NoteType)  ReverseProperties.getInstance().getMModelServices().getModelFactory().createElement("NoteType");
    ...
    private static TagType createTagType(final Stereotype stereotype, final Property ecoreElement) {
        ...
        TagType attr = (TagType) ReverseProperties.getInstance().getMModelServices().getModelFactory().createElement("TagType");
    ...
    private static void createNoteType(final MetaclassReference reference, final Property ecoreElement) {
        ...
        if (ids.size()>0) {
            String id = ids.get(0);
            noteType = (NoteType) ReverseProperties.getInstance().getMModelServices().findById(SmClass.getClass(NoteType.class), UUID.fromString(id));
        }

        if (noteType == null){
            noteType = (NoteType) ReverseProperties.getInstance().getMModelServices().getModelFactory().createElement("NoteType");
    ...
    private static TagType createTagType(final MetaclassReference reference, final Property ecoreElement) {
        ...
        if (ids.size() > 0){
            tagType = (TagType) ReverseProperties.getInstance().getMModelServices().findById(SmClass.getClass(TagType.class), UUID.fromString(ObjingEAnnotation.getObjingIDs(ecoreElement).get(0)));
            ...
            if ((name != null ) && (!name.equals("")) 
                    ...
                    && (tagType == null)){
                tagType = (TagType) ReverseProperties.getInstance().getMModelServices().getModelFactory().createElement("TagType");
            ...
            if (!file.exists()){
                ...
                try{
                    ...
                    if (e.getMessage().equals("Unsupported or unrecognized format")){
                        ...
                        xmilogs.writelnInLog(Xmi.I18N.getMessage("logFile.warning.import.unsupportedFormat", location, format));
                    }else{
    ...
    public static Profile getProfileOwner(final Profile profile) {
        ...
        if (name.contains(nameSpacingSeparator)){
            ...
            for (int  i = 1 ;  i < size - 1; i++ ){
                ownerName = ownerName + nameSpacingSeparator + names[i];              
            }
    ...
    public static org.eclipse.uml2.uml.Profile createEcoreProfile(final Profile objingElt) {
        ...
        if (SysMLProfileUtils.isSysML(objingElt)){
            ...
            SysMLProfileUtils.completeSysMLprofile(ecoreProfile);
