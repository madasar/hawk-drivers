...
public class SysMLProfileUtils {
    ...
    private static void addVerifyStereotype(final org.eclipse.uml2.uml.Profile sysMLProfile) {
        ...
        try {
            Stereotype obStereotype = GenerationProperties.getInstance().getMModelServices()
                    .getStereotype(XMIProperties.modelerModuleName, "verify", SmClass.getClass(Dependency.class));
    ...
    private static void addSatisfyStereotype(final org.eclipse.uml2.uml.Profile sysMLProfile) {
        ...
        try {
            Stereotype obStereotype = GenerationProperties.getInstance().getMModelServices()
                    .getStereotype(XMIProperties.modelerModuleName, "satisfy",  SmClass.getClass(Dependency.class));
            TotalExportMap.getInstance().put(obStereotype.getUuid().toString(), stereotype);
        } catch (IllegalArgumentException | ElementNotUniqueException e) {
            Xmi.LOG.warning(e);
        }
    ...
    private static void addDeriveStereotype(final org.eclipse.uml2.uml.Profile sysMLProfile) {
        ...
        try {
            Stereotype obStereotype = GenerationProperties.getInstance().getMModelServices()
                    .getStereotype(XMIProperties.modelerModuleName, "derive",  SmClass.getClass(Dependency.class));
            TotalExportMap.getInstance().put(obStereotype.getUuid().toString(), stereotype);
        } catch (IllegalArgumentException | ElementNotUniqueException e) {
            Xmi.LOG.warning(e);
        }
    ...
    private static void addRequirementStereotype(final org.eclipse.uml2.uml.Profile sysMLProfile) {
        ...
        Property text = stereotype.createOwnedAttribute("Text", GenerationProperties.getInstance().getEcoreUMLTypes().getString());
        text.setLower(1);
        text.setUpper(1);
        
        Property id = stereotype.createOwnedAttribute("Id",  GenerationProperties.getInstance().getEcoreUMLTypes().getString());
        id.setLower(1);
        id.setUpper(1);
    }
