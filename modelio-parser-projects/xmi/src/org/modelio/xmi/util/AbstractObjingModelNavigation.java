...
public abstract class AbstractObjingModelNavigation {
    ...
    public static MObject getBindableInstanceOwner(BindableInstance instance) {
    ...
    public static boolean mustBeExported(Stereotype obStereotype) {
        ...
        if (profile.getOwnerModule().getName().equals(XMIProperties.modelerModuleName)  && profile.getName().equals("default") 
        ...
        if (objingElt instanceof BindableInstance){
            ...
            if ((listProperty != null) && (listProperty.size() > 0) ){
                ...
                if ((base instanceof ModelElement) && (!base.equals(objingElt)))
                    ...
                    ((BindableInstance) objingElt).setRepresentedFeature(((List<? extends ModelElement>) base).get(0));
                }
            }
        }
    }
    ...
    public static boolean isValidConnector(Link link) {
        ...
        for (LinkEnd connectorEnd : link.getLinkEnd()){
            MObject temp = connectorEnd.getOwner();
    ...
    public static Element getConnectorOwner(Link connector) {
        Element firstOwner = null;
        Element secondOwner = null;
        
        for (LinkEnd connectorEnd : connector.getLinkEnd()){
          
            secondOwner = connectorEnd.getOwner();
